...
#include "Weapon Logic/Pistol.h"
#include "Camera/CameraComponent.h"

void APistol::FireWeapon()
{
	OnFireWeapon();
	//Use primary fire from fps project here adapted into c++

}

bool APistol::InitialRaycast(int damage, float range, UCameraComponent* PlayerCamera, FHitResult& Hit)
{

    FVector StartPosition = PlayerCamera->GetComponentTransform().GetLocation(); //Get World Location
    FVector EndPosition = PlayerCamera->GetComponentTransform().GetLocation() + PlayerCamera->GetForwardVector() * range;
    
    FHitResult OutHit;
    FCollisionQueryParams Params;
    Params.AddIgnoredActor(this);
    Params.AddIgnoredActor(GetWorld()->GetFirstPlayerController()->GetPawn());      //Game Trace Channel 11 is WeaponsChannel, OR MAYBE NOT
    if (GetWorld()->LineTraceSingleByChannel(OutHit, StartPosition, EndPosition, ECC_GameTraceChannel11, Params))
    {
        Hit = OutHit;
        return true;
    }
